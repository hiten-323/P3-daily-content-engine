...
5. **Fix Growth Reel Publishing (P0)**
   ...
   - In `run_publish_slot` for the `"evening"` slot, change the reel fetching from `reels = content.get("reels") or []` and `reel  = reels[0] if reels and isinstance(reels[0], dict) else {}` to `reel = content.get("growth_reel") or {}`.
...
8. **Fix Account-Level Attribution (P0)**
   - In `content_generator/analytics/insights_fetcher.py`, in `fetch_pending_insights`, remove the lines:
     ```python
     if due:
